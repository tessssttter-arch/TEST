 ...
 * Удваивает кавычки и оборачивает значение в кавычки, если есть спецсимволы.
 */
function escapeCSVCell(val: any): string {
  ...
  // Если нет спецсимволов, можно не оборачивать в кавычки
  const needsQuotes = /[",\r\n]/.test(stringValue);
...
export function exportPostsToCSV(posts: VKPost[], selectedColumns: string[]): string {
  ...
  // 1. Создаем строку заголовков БЕЗ экранирования (заголовки простые)
  const headers = selectedColumns.map((col) => {
    return EXPORT_COLUMNS_MAP[col] || col;
  });
