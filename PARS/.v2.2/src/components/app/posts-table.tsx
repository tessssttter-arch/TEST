...
import { VKPost, VKImage } from '../../types/vk';
...
export const PostsTable: React.FC = () => {
  ...
  const renderSortIndicator = (key: 'date' | 'likes' | 'comments' | 'reposts' | 'views' | 'price') => {
    if (sortConfig.key !== key) {
      return <span className="text-slate-350 dark:text-slate-700 ml-1 select-none font-normal">⇅</span>;
    }
  ...
  if (filteredPosts.length === 0) {
    return (
      <div id="posts-empty-state" className="flex flex-col items-center justify-center p-12 text-center bg-slate-50 dark:bg-slate-900/10 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl">
    ...
    <div id="posts-table-wrapper" className="w-full border border-slate-200 dark:border-slate-800 rounded-2xl bg-white dark:bg-slate-900 overflow-hidden shadow-xs">
      ...
      <TableVirtuoso
          ...
          return (
            <tr className="bg-slate-50 dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 text-[11px] font-bold tracking-wider uppercase text-slate-500 text-left select-none">
              <th className="p-3 w-10 text-center">
                <input
                  ...
                  className="w-4 h-4 text-indigo-650 bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-700 rounded-xs cursor-pointer accent-indigo-600"
                />
              </th>
            ...
            {isColVisible('metrics') && (
              <th className="p-3 w-44 text-left select-none">
                <div className="text-slate-450 text-[9px] font-semibold mb-1 uppercase tracking-wider">
        ...
        itemContent={(index, post) => {
          const isRowExported = post.is_exported;
          const isSelected = selectedPostIds.has(post.post_id);
          return (
            <>
              {/* Специфический Чекбокс Выбора */}
              <td className="p-3 text-center align-middle border-b border-slate-100 dark:border-slate-800/65">
                <input
                  type="checkbox"
                  checked={isSelected}
                  onChange={() => {
              ...
              {isColVisible('favorite') && (
                <td className="p-3 text-center align-middle border-b border-slate-100 dark:border-slate-800/65">
                  <button
                    ...
                    className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-all cursor-pointer"
                  >
                    <Star
                      size={16}
              ...
              {isColVisible('thumbnail') && (
                <td className="p-3 align-middle border-b border-slate-100 dark:border-slate-800/65">
                  {post.images.length > 0 ? (
                    <div className="relative group w-12 h-12 rounded-lg bg-slate-50 dark:bg-slate-950 overflow-hidden cursor-pointer shadow-2xs border border-slate-105" onClick={() => openMediaModal(post, 0)}>
                      <img
                        loading="lazy"
                  ...
                  <div className="flex items-center gap-2 max-w-[170px]">
                    ...
                    <div className="min-w-0">
                      ...
                      <a
                        ...
                        className="inline-flex items-center gap-0.5 text-[10px] text-indigo-600 hover:text-indigo-805 dark:text-indigo-400 hover:underline"
                      >
                        wall_{post.id}
                        <ExternalLink size={8} />
                      </a>
                  ...
                  {editingTextPostId === post.post_id ? (
                    <div className="space-y-2">
                      <textarea
                        ...
                        rows={4}
                        className="w-full text-xs p-2 rounded-lg border border-indigo-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-100 outline-hidden outline-hidden resize-y font-normal"
                      />
                      <div className="flex gap-1.5 justify-end">
                        <button
                          type="button"
                          onClick={() => setEditingTextPostId(null)}
                          className="px-2.5 py-1 text-[10px] font-bold text-slate-500 hover:text-slate-650 bg-slate-50 hover:bg-slate-100 rounded-md cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 text-[10px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-md cursor-pointer"
                        >
                    ...
                    <div className="text-xs text-slate-700 dark:text-slate-300">
                      <div className="whitespace-pre-line leading-relaxed font-normal">
                        {expandedTextIds.has(post.post_id) || post.text_edited.length <= 180 ? (
                          ...
                          <>
                            ...
                            <button
                              ...
                              className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline py-0 inline cursor-pointer"
                            >
                              читать далее
                            </button>
                          </>
                        ...
                        {expandedTextIds.has(post.post_id) && post.text_edited.length > 180 && (
                          <button
                            type="button"
                            onClick={() => toggleTextExpand(post.post_id)}
                            className="text-slate-400 font-bold hover:underline py-0 inline ml-1.5 cursor-pointer"
                          >
                            свернуть
                          </button>
                        )}
                      ...
                      <button
                        ...
                        className="mt-2 inline-flex items-center gap-1 text-[10px] font-medium text-slate-400 hover:text-indigo-600 hover:bg-indigo-50/50 dark:hover:bg-indigo-950/20 px-1.5 py-0.5 rounded cursor-pointer"
                      >
                        <Edit2 size={8} />
                        Править описание
                      </button>
                ...
                <td className="p-3 align-middle border-b border-slate-100 dark:border-slate-800/65">
                  {post.price_original !== null ? (
                    <div className="text-xs">
                      ...
                      {post.price_details && post.price_details.currency !== 'RUB' && (
                        <p className="text-[9px] text-slate-400 font-semibold uppercase mt-0.5">
                          из {post.price_details.originalValue} {post.price_details.currency}
                        </p>
                      )}
                    </div>
                  ) : (
                  ...
                  {editingPricePostId === post.post_id ? (
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-1 justify-end">
                        <button
                          ...
                          className="p-1 px-1.5 text-[8px] font-extrabold text-slate-500 hover:bg-slate-100 rounded cursor-pointer"
                        >
                          <X size={8} />
                        </button>
                        <button
                          ...
                          className="p-1 px-1.5 text-[8px] font-extrabold text-white bg-indigo-600 rounded cursor-pointer"
                        >
                          <Check size={8} />
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-2 group/price text-xs">
                      ...
                      <button
                        ...
                        className="p-1 text-slate-350 hover:text-indigo-600 hover:bg-slate-50 rounded hidden group-hover/price:block cursor-pointer transition-all"
              ...
              {/* Колонка 8: Дата */}
              {isColVisible('date') && (
                <td className="p-3 align-middle text-xs font-medium text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800/65">
                  {post.date_formatted}
                </td>
              )}
                  ...
                  <button
                    ...
                    className={`inline-flex items-center justify-center p-1 px-2.5 text-[10px] font-extrabold rounded-md border transition-all cursor-pointer ${
                      post.is_exported
                        ...
                        : 'bg-white border-slate-200 text-slate-400 hover:text-slate-650 hover:bg-slate-50 dark:bg-slate-905 dark:border-slate-700'
                    }`}
                  >
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 bg-slate-900 text-white rounded-2xl flex items-center justify-between gap-6 p-4 px-6 shadow-xl border border-slate-800 animate-in slide-in-from-bottom-5 duration-200 max-w-[90vw] md:max-w-xl">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="py-1.5 px-3 bg-slate-800 hover:bg-slate-750 text-white font-bold text-xs rounded-lg border border-slate-700 transition-all cursor-pointer flex items-center gap-1 active:scale-[0.98]"
            >
              <Star size={11} className="text-amber-500 fill-amber-500 hover:scale-105" />
              В избранное
            </button>
            <button
              onClick={handleBulkExportToVk}
              className="py-1.5 px-4 bg-[#4C75A3] hover:bg-[#3d618a] text-white font-bold text-xs rounded-lg transition-all cursor-pointer flex items-center gap-1 active:scale-[0.98]"
            >
              🚀 Выгрузить в VK
            </button>
            <button
              onClick={() => setSelectedPostIds(new Set())}
              className="p-1.5 hover:bg-slate-800/80 text-slate-400 hover:text-white rounded-lg transition-all cursor-pointer"
      ...
      {isPublishing && (
        <div id="publish-progress-overlay" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-xl w-full border border-slate-205 dark:border-slate-800 shadow-2xl p-6 flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-150">
            {/* Header */}
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-4 mb-4">
              ...
              {!publishSummary && (
                <div className="flex items-center gap-2">
                  <Loader2 size={13} className="animate-spin text-indigo-505" />
                  <span className="text-[10px] text-slate-450 font-bold uppercase tracking-wider">Работаем...</span>
                </div>
              )}
            </div>
            ...
            {!publishSummary && activePublishPost && (
              <div className="mb-4 bg-slate-50 dark:bg-slate-950/40 border border-slate-205 dark:border-slate-800 p-3 rounded-xl flex gap-3 animate-in fade-in">
                {activePublishPost.images.length > 0 && (
                  <img src={activePublishPost.images[0].url} alt="pic" className="w-11 h-11 object-cover rounded-md shadow-xs border border-slate-255" />
                )}
                <div className="min-w-0">
                  <span className="text-[9px] font-bold text-[#4C75A3]">Публикуется товар #{publishingProgress.current}:</span>
                  <p className="text-xs text-slate-850 dark:text-slate-250 truncate leading-relaxed">
                    "{activePublishPost.text_edited || activePublishPost.text_original}"
                  </p>
                  <p className="text-[10px] text-slate-400 font-bold mt-1">Цена: {activePublishPost.price_edited} руб.</p>
                </div>
              </div>
            )}
            ...
            <div className="space-y-1.5 mb-4">
              ...
              <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden border border-slate-205 dark:border-slate-705">
                <div 
                  className="bg-gradient-to-r from-[#4C75A3] to-[#5B88BD] h-full transition-all duration-300"
                  style={{ width: `${(publishingProgress.current / publishingProgress.total) * 100}%` }}
                />
              </div>
            </div>

            {/* Консоль логов */}
            <div className="flex-1 bg-slate-955 text-slate-200 p-3.5 rounded-xl border border-slate-800 font-mono text-[10px] overflow-y-auto max-h-[160px] flex flex-col space-y-1.5">
            ...
            {/* Сводка результатов в конце */}
            {publishSummary && (
              <div className="mt-4 p-3 bg-emerald-50 dark:bg-emerald-950/25 border border-emerald-100 dark:border-emerald-900 rounded-xl space-y-1.5 animate-in slide-in-from-top-2">
            ...
            {publishSummary && (
              <button
                ...
                className="mt-4 w-full py-2.5 px-4 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl transition-all cursor-pointer"
              >
                Вернуться к таблице
              </button>
            )}
      ...
      {activeMediaPost && (
        <div id="media-carousel-overlay" className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-3xl bg-slate-900 rounded-2xl overflow-hidden shadow-2xl flex flex-col max-h-[85vh] z-10 animate-in zoom-in-95 duration-150">
            {/* Шапка модалки */}
            <div className="flex items-center justify-between px-6 py-4 bg-slate-950 text-white border-b border-slate-800">
              ...
              <button
                ...
                className="p-1 px-2 hover:bg-slate-800 text-slate-400 hover:text-white rounded-lg cursor-pointer"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 relative flex items-center justify-center bg-slate-950 p-6 min-h-[300px] max-h-[60vh]">
              {activeMediaPost.images.length > 0 ? (
                <img
                  ...
                  className="max-w-full max-h-[50vh] object-contain rounded-lg animate-in fade-in duration-305"
                />
              ) : (
              ...
              {activeMediaPost.images.length > 1 && (
                <>
                  <button
                    type="button"
                    onClick={prevMedia}
                    className="absolute left-4 p-2 rounded-full bg-slate-900/80 hover:bg-slate-800 text-white shadow-md cursor-pointer"
                  >
                    <ChevronLeft size={18} />
                  </button>
                  <button
                    type="button"
                    onClick={nextMedia}
                    className="absolute right-4 p-2 rounded-full bg-slate-900/80 hover:bg-slate-800 text-white shadow-md cursor-pointer"
                  >
