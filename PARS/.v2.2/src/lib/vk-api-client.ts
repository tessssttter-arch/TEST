...
export function generateSandboxWall(ownerId: number, count: number = 30, priceConfig: PriceConfig): VKPost[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Формируем картинки (1 до 4 штук)
...
export const VK_API = {
  ...
  async exportPostToGroup(
    ...
    if (isSandbox) {
      ...
      if (privacySettings) {
        onProgress(`🛡️ Применение приватных фильтров: Друзьям: ${privacySettings.friendsOnly ? 'Да' : 'Нет'}, Закрыть кавычки комментариев: ${privacySettings.closeComments ? 'Да' : 'Нет'}, Без пушей: ${privacySettings.muteNotifications ? 'Да' : 'Нет'}`);
    ...
    try {
      ...
      // Требуется метод wall.post. owner_id сообщества должен быть отрицательным
