...
- [x] **[AI-EDIT] Обратный импорт правок из Excel (CSV) | 2026-06-16**
 ...
 - [x] **[AI-EDIT] Интеграционный чекпоинт | 2026-06-20**
   - Сверить экспорт JSON v3 в `src/components/app/export-dialog.tsx` с источником экспорта. Устранен несуществующий импорт `exportToCatalog`, заменен на прямой вызов `exportPostsToCatalogJsonV3` из `src/lib/catalog-exporter.ts:89`. `usePostsStore` также экспортирует совместимый метод `exportToCatalog`. Проверка `tsc --noEmit` прошла.
