      ...
      <header className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-6">
        <div className="space-y-1 text-left">
          <div className="flex items-center gap-2">
            <span className="px-2.5 py-1 text-xs font-semibold text-white bg-indigo-600 rounded-lg shadow-sm">
              VK Insights v1.1
            </span>
          </div>
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white sm:text-4xl text-left">
            VK Insights Analyzer
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 max-w-2xl leading-normal text-left">
      ...
      {posts.length > 0 ? (
        <div id="analyzer-dashboard-grid" className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
          ...
          <section id="table-explorer-section" className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer border-0 ${
                      viewMode === 'gallery'
                        ? 'bg-white dark:bg-slate-900 text-indigo-705 shadow-xs'
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-505 dark:hover:bg-indigo-650 disabled:opacity-50 disabled:cursor-not-allowed rounded-xl shadow-xs hover:shadow-sm cursor-pointer transition-all border-0"
                >
                  <Download size={15} />
            ...
            <div className="flex items-start gap-2.5 p-3.5 rounded-xl border border-blue-50 dark:border-blue-950 bg-blue-50/30 dark:bg-blue-950/10 text-[11px] text-blue-600 dark:text-blue-400 text-left">
              <Info size={14} className="shrink-0 mt-0.5" />
              <div>
                <p className="font-semibold leading-normal text-left">Локальное слияние правок</p>
                <p className="leading-relaxed opacity-90 mt-0.5 text-left">
        ...
        <div id="instructions-card" className="p-6 bg-slate-50 dark:bg-slate-900/10 border border-slate-205 dark:border-slate-800 rounded-2xl max-w-3xl mx-auto space-y-4">
          <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
            <Sparkles size={18} />
            <span className="text-xs font-bold uppercase tracking-wider text-left block">Как это работает?</span>
          </div>
          <div className="space-y-2 text-slate-600 dark:text-slate-400 text-xs leading-relaxed font-normal text-left">
            ...
            <div className="space-y-1 pl-4 my-2 list-decimal list-inside text-left">
              ...
              <div>5. Сами отберите нужные столбцы, пресеты экспорта и скачайте сформированную таблицу в Excel-совместимый <code className="font-mono text-slate-500 text-left">.csv</code> файл.</div>
            </div>
          </div>
        </div>
      )}
