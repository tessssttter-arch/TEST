...
export const FileUploader: React.FC = () => {
  ...
  return (
    <div id="file-uploader-section" className="w-full text-left">
      {posts.length > 0 && fileName ? (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 rounded-xl border border-emerald-100 bg-emerald-50/50 dark:border-emerald-950/30 dark:bg-emerald-950/10 text-left">
          <div className="flex items-center gap-3">
            <div className="p-2.5 bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400 rounded-lg">
              <FileCode size={20} />
            </div>
            <div className="text-left">
              <p className="font-semibold text-sm text-slate-800 dark:text-slate-200 text-left">{fileName}</p>
              <p className="text-xs text-slate-500 text-left">Успешно загружено и структурировано {posts.length} постов.</p>
            </div>
          </div>
          <button
            id="reupload-button"
        ...
        >
          ...
          {activeError && (
            <div
              ...
              className="absolute -bottom-14 left-0 right-0 p-3 flex items-center gap-2 bg-rose-50 text-rose-700 dark:bg-rose-950/20 dark:text-rose-400 border border-rose-200 dark:border-rose-900/30 rounded-xl max-w-lg mx-auto shadow-xs text-xs animate-in fade-in slide-in-from-top-2 duration-300 text-left"
            >
              <AlertCircle size={16} className="shrink-0" />
              <span className="text-left">{activeError}</span>
            </div>
          )}
        </div>
      )}
