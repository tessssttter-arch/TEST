...
export const TextIntelligence = {
  ...
  parse(rawPost: { id: number; owner_id: number; text: string; attachments?: any[]; date: number }): SmartParsedPost {
    ...
    // 4. Извлечение цен (с учетом ID поставщика)
    const priceAnalysis = extractAndClassifyPrices(originalText, rawPost.owner_id);
    addTrace(
      'Извлечение цен',
    ...
    // 5. Размеры (с учетом ID поставщика)
    const sizes = extractSizes(originalText, rawPost.owner_id);
    if (sizes.length > 0) {
   ...
   */
  splitMultiPosts(parsed: SmartParsedPost, ownerId?: string | number): SmartParsedPost[] {
    ...
    // Если найдено по крайней мере два чек-пойнта/раздела с нумерацией и в тексте есть разные цены
    if (matches.length >= 2 && parsed.images.length >= 2) {
      ...
      matches.forEach((itemText, idx) => {
        ...
        const subPriceAnalysis = extractAndClassifyPrices(itemText, ownerId);
        ...
        splits.push({
          ...
          sizes: extractSizes(itemText, ownerId).length > 0 ? extractSizes(itemText, ownerId) : parsed.sizes,
  ...
  /**
   * Разбор размеров по тексту
   */
  extractSizes(text: string, ownerId?: string | number): string[] {
    return extractSizes(text, ownerId);
  }
};
