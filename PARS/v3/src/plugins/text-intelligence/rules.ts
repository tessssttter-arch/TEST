 ...
 */
export function extractSizes(text: string, ownerId?: string | number): string[] {
  const normalized = text.toLowerCase();
  
  // Сначала проверяем шаблоны размеров поставщика
  if (ownerId !== undefined) {
    const oId = String(ownerId);
    try {
      const savedPatternsJson = localStorage.getItem('vk_supplier_patterns');
      if (savedPatternsJson) {
        const patternsMap = JSON.parse(savedPatternsJson);
        const supplierPattern = patternsMap[oId];
        if (supplierPattern && supplierPattern.sizePattern) {
          const customRegex = new RegExp(supplierPattern.sizePattern, 'ig');
          const sizesFound: Set<string> = new Set();
          let match;
          while ((match = customRegex.exec(normalized)) !== null) {
            const sizeVal = match[1] || match[0];
            if (sizeVal) {
              sizesFound.add(sizeVal.trim().toUpperCase());
            }
          }
      ...
      console.error('Ошибка сопоставления шаблона размеров поставщика:', e);
    }
  }
 ...
 */
export function extractAndClassifyPrices(text: string, ownerId?: string | number): ExtractedPrices {
  ...
  // Сначала проверяем пользовательские регулярные выражения для цен поставщика
  if (ownerId !== undefined) {
    const oId = String(ownerId);
    try {
      const savedPatternsJson = localStorage.getItem('vk_supplier_patterns');
      if (savedPatternsJson) {
        const patternsMap = JSON.parse(savedPatternsJson);
        const supplierPattern = patternsMap[oId];
        if (supplierPattern && supplierPattern.pricePattern) {
          const customRegex = new RegExp(supplierPattern.pricePattern, 'i');
          const customMatch = customRegex.exec(text); // Работаем с исходным регистром, регулярка с 'i'
          if (customMatch) {
            // Берем группу захвата (если есть) или полное совпадение
            const capturedPriceStr = customMatch[1] || customMatch[0];
            // Извлекаем только цифры из строки
            const numeralsOnly = capturedPriceStr.replace(/\D/g, '');
            const parsedCustomPrice = parseFloat(numeralsOnly);
            if (!isNaN(parsedCustomPrice) && parsedCustomPrice >= 100 && parsedCustomPrice <= 150000) {
              result.retail = parsedCustomPrice;
              result.rawList.push({ 
                value: parsedCustomPrice, 
                label: 'custom_supplier_pattern', 
                index: customMatch.index 
              });
              return result; // Перехватываем стандартную обработку и возвращаем результат
            }
          }
        }
      }
    } catch (e) {
      console.error('Ошибка применения шаблона цены поставщика:', e);
    }
  }
