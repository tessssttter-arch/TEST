...
import { VK_API } from '../../lib/vk-api-client';
...
export const VkIntegration: React.FC = () => {
  ...
  }, [activeSess, autoExportAll, closeComments, markAsAds, monitoringEnabled, monitoringInterval, muteNotifications]);
  ...
  return (
    <div id="vk-integration-control-panel" className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-805 rounded-2xl overflow-hidden shadow-xs">
      
      {/* Шапка секции */}
      <div className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-left">
        <div className="flex items-center gap-2.5">
          <div className="p-2 bg-[#4C75A3] text-white rounded-lg shadow-xs">
            <Radio size={18} className="animate-pulse" />
          </div>
          <div className="text-left">
            <h2 className="text-sm font-extrabold text-[#1f2937] dark:text-white uppercase tracking-wider">
        ...
        {vkAccount && (
          <span className={`self-start sm:self-auto flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold rounded-full ${
            vkAccount.isSandbox 
              ...
              : 'bg-emerald-100 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-400 border border-emerald-250'
          }`}>
            <Database size={11} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 divide-y lg:divide-y-0 lg:divide-x divide-slate-200 dark:divide-slate-800">
        ...
        <div className="lg:col-span-4 p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="text-xs text-indigo-650 hover:text-indigo-800 font-bold flex items-center gap-0.5 border-0 bg-transparent cursor-pointer"
            >
              <Plus size={14} />
              <span>Добавить</span>
            </button>
          ...
          {showAddSessionForm && (
            <div className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in duration-200">
              <div className="flex bg-slate-200/60 dark:bg-slate-800 p-1 rounded-lg">
                <button
                  type="button"
                  onClick={() => { setAuthMode('sandbox'); setParseStatus(null); }}
                  className={`flex-1 py-1 px-3 text-[10px] font-bold rounded-md transition-all cursor-pointer border-0 ${
                    authMode === 'sandbox' ? 'bg-white dark:bg-slate-900 text-[#4C75A3] shadow-xs' : 'text-slate-500 bg-transparent'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-1 px-3 text-[10px] font-bold rounded-md transition-all cursor-pointer border-0 ${
                    authMode === 'real' ? 'bg-white dark:bg-slate-900 text-[#4C75A3] shadow-xs' : 'text-slate-500 bg-transparent'
                  }`}
                >
              ...
              {authMode === 'sandbox' ? (
                <div className="space-y-3 text-left">
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-1.5 py-2 px-3 bg-[#4C75A3] hover:bg-[#3d618a] text-white font-bold text-xs rounded-lg cursor-pointer border-0"
                  >
                    <UserCheck size={13} />
                ...
                <form onSubmit={handleAddRealToken} className="space-y-3 text-left">
                  <div className="flex items-center justify-between text-[10px] font-bold">
                    <span className="text-slate-600">VK Access Token:</span>
                    <button
                      type="button"
                      onClick={() => setShowHelper(!showHelper)}
                      className="text-[#4C75A3] flex items-center gap-0.5 border-0 bg-transparent cursor-pointer"
                    >
                      <HelpCircle size={10} />
                      Инструкция
                    </button>
                  ...
                  <input
                    ...
                    className="w-full text-xs p-2 rounded-lg border bg-white dark:bg-slate-900 font-mono text-left"
                  />

                  <button
                    type="submit"
                    className="w-full flex items-center justify-center gap-1.5 py-2 px-3 bg-[#4C75A3] text-white font-bold text-xs rounded-lg border-0 cursor-pointer"
                  >
                    <Key size={13} />
          ...
          <div className="space-y-2 text-left">
            {sessions.length > 0 ? (
                ...
                return (
                  <div
                    ...
                    className={`group relative flex items-center justify-between p-3 rounded-xl border transition-all cursor-pointer ${
                      isActive 
                        ? 'bg-indigo-55 border-indigo-200 dark:bg-indigo-950/10' 
                        : 'bg-white dark:bg-slate-900 border-slate-200 hover:bg-slate-50'
                    }`}
                  >
                    ...
                    <button
                      ...
                      className="p-1 rounded opacity-0 group-hover:opacity-100 text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition-opacity border-0 bg-transparent cursor-pointer"
                    >
                      <Trash2 size={13} />
                    </button>
                  </div>
              ...
              <div className="p-6 text-center border border-dashed rounded-xl bg-slate-50/50 text-slate-450">
                ...
                <button
                  ...
                  className="mt-1.5 px-3 py-1 bg-[#4C75A3] text-white text-[9px] font-black rounded border-0 cursor-pointer"
                >
          ...
          {vkAccount && (
            <div className="pt-2 border-t border-slate-100 dark:border-slate-800 space-y-1.5 text-left">
              <div className="flex items-center justify-between text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <button type="button" onClick={loadGroups} className="text-[#4C75A3] border-0 bg-transparent cursor-pointer">
              ...
              {vkGroups.length > 0 ? (
                <select
                  ...
                  className="w-full text-xs p-2 rounded-lg border bg-slate-50 dark:bg-slate-900 font-bold text-slate-800"
                >
                  {vkGroups.map((g) => (
                ...
                <div className="p-2.5 text-[10px] text-amber-600 bg-amber-50 dark:bg-slate-850 rounded-lg text-left">
        ...
        <div className="lg:col-span-5 p-5 space-y-4">
          ...
          {showPresetCreator ? (
            ...
            <form onSubmit={handleCreatePreset} className="p-4 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-205 dark:border-slate-800 space-y-3 text-left animate-in fade-in duration-200">
              ...
              <div className="space-y-1 text-left">
                ...
                <input
                  ...
                  className="w-full text-xs p-2 border bg-white dark:bg-slate-900 rounded text-left"
                />
              </div>
              ...
              <div className="space-y-1 text-left">
                ...
                <textarea
                  ...
                  className="w-full text-xs p-2 border bg-white dark:bg-slate-900 rounded font-mono text-left"
                />
              </div>
              ...
              <div className="flex gap-2 pt-1">
                <button
                  type="submit"
                  className="flex-1 py-1.5 bg-indigo-600 hover:bg-indigo-750 text-white text-xs font-bold rounded border-0 cursor-pointer"
                >
                  Добавить Пресет
                </button>
                <button
                  ...
                  className="px-2.5 py-1.5 bg-slate-200 text-slate-605 text-xs rounded border-0 cursor-pointer"
                >
                  Отмена
                </button>
              </div>
            ...
            <form onSubmit={handleFetchWall} className="space-y-4 text-left">
              <div className="space-y-1.5 text-left">
                ...
                <div className="flex gap-1.5">
                  <select
                     ...
                     className="flex-1 text-xs p-2.5 rounded-lg border bg-slate-50 dark:bg-slate-900 font-bold text-slate-800"
                  >
                    {presets.map((p) => (
                  ...
                  {presets.length > 2 && (
                    <button
                      ...
                      className="p-2 border rounded-lg hover:border-rose-300 hover:text-rose-500 hover:bg-rose-50 cursor-pointer bg-white dark:bg-slate-900 text-slate-550"
                    >
                      <Trash2 size={14} />
                    </button>
                  )}
              ...
              <div className="space-y-1 text-left">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full text-xs p-2.5 pl-8 rounded-lg border bg-slate-50 dark:bg-slate-900 font-bold font-mono text-indigo-700 dark:text-indigo-300 text-left"
                  />
                  <Search size={13} className="absolute left-2.5 top-3.5 text-slate-400" />
                </div>
              </div>

              <div className="space-y-1 bg-slate-50 dark:bg-slate-950 p-2.5 rounded-lg border border-slate-205 dark:border-slate-850">
                ...
                <input
                  ...
                  className="w-full h-1 bg-slate-200 dark:bg-slate-800 rounded appearance-none cursor-pointer accent-indigo-600 mt-2 border-0"
                />
              </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-3 px-4 bg-slate-900 hover:bg-slate-800 disabled:opacity-40 disabled:cursor-not-allowed text-white font-extrabold text-xs rounded-xl shadow-xs transition-colors border-0 cursor-pointer"
              >
          ...
          {/* Статус- бар */}
          {parseStatus && (
            <div className="p-3 text-[10px] bg-slate-50 dark:bg-slate-800 border-l-2 border-[#4C75A3] text-slate-705 dark:text-slate-350 font-semibold rounded-r-lg text-left animate-pulse">
              {parseStatus}
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-3 p-5 flex flex-col h-full space-y-3.5 text-left">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
          ...
          <div className="flex-1 overflow-y-auto max-h-[220px] border border-slate-200 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950/20 p-2.5 space-y-2 font-mono text-[9px] text-left">
            {exportLogs.length > 0 ? (
              exportLogs.map((log) => (
                <div 
                  key={log.id} 
                  className={`p-2 rounded border leading-normal text-left ${
                    log.status === 'success' 
                      ...
                      : 'bg-rose-50/40 border-rose-100 text-rose-800 dark:bg-rose-950/10 dark:border-rose-900/20 dark:text-rose-450'
                  }`}
                >
                  ...
                  <p className="line-clamp-2 text-slate-500 dark:text-slate-400 italic mb-1 text-left">
                    "{log.postText}"
                  </p>
                  <div className="flex items-center justify-between text-[8px] font-bold uppercase tracking-wider mt-1 border-t border-dotted border-current/20 pt-1">
                    ...
                    <span className="flex items-center gap-0.5">
                      {log.status === 'success' ? 'Опубликован' : 'Ошибка'}
                    </span>
                  </div>
                  {log.status === 'success' && (
                    <a 
      ...
      {vkAccount && activeSess && (
        <div id="smm-bot-rules-panel" className="border-t border-slate-200 dark:border-slate-800 p-5 bg-slate-50/20 dark:bg-slate-950/20 text-left">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800 mb-5">
            <div className="text-left">
              <h3 className="text-sm font-extrabold text-[#1f2937] dark:text-white flex items-center gap-2">
                <Settings size={16} className="text-[#4C75A3]" />
            ...
            <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-slate-200 dark:border-slate-800 shrink-0">
              <button
                ...
                className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all cursor-pointer border-0 ${
          ...
          {activeConfigTab === 'individual' ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-205">
              {/* Конфигурация Приватности публикации */}
              <div className="space-y-3.5 bg-white dark:bg-slate-950/40 p-4 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xs text-left">
                ...
                <div className="space-y-2.5 text-left">
                  ...
                  <label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
                    <input
                      ...
                      className="rounded text-indigo-600 dark:bg-slate-900 border-slate-300"
                    />
                    <span>Без Push-оповещений (тихо)</span>
                  </label>
                  <label className="flex items-center gap-2 text-xs font-bold text-slate-700 dark:text-slate-300 cursor-pointer">
                    <input
                      type="checkbox"
                      ...
                      className="rounded text-indigo-600 dark:bg-slate-900 border-slate-300"
                    />
                    <span>Пометить как рекламу</span>
                  </label>
                </div>
              </div>

              {/* Наложения контента: водяные знаки, подпись */}
              <div className="space-y-3.5 bg-white dark:bg-slate-950/40 p-4 border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xs text-left">
                <h4 className="text-[11px] font-extrabold uppercase text-[#4C75A3] flex items-center gap-1.5">
                ...
                <div className="space-y-3 text-left">
                  ...
                  <div className="space-y-1 text-left">
                    <span className="text-[10px] font-extrabold text-slate-500 uppercase block">Шаблон индивидуальной подписи:</span>
                    <textarea
                      rows={2}
                      value={textSignature}
                      onChange={(e) => {
                      ...
                      className="w-full text-xs p-2 border bg-slate-50 dark:bg-slate-900 dark:border-slate-800 rounded font-bold text-slate-800 dark:text-slate-100"
                    />
                  </div>
                </div>
              </div>
                ...
                <div className="space-y-3 text-left">
                  <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-lg border dark:border-slate-800">
                    <button
                      ...
                      className={`flex-1 py-1 text-[10px] font-bold rounded cursor-pointer border-0 ${
                        priceMarkupType === 'percent' ? 'bg-white dark:bg-slate-800 text-indigo-650' : 'text-slate-500 bg-transparent'
                      }`}
                    >
                      Наценка в %
                    </button>
                    <button
                      ...
                      className={`flex-1 py-1 text-[10px] font-bold rounded cursor-pointer border-0 ${
                        priceMarkupType === 'fixed' ? 'bg-white dark:bg-slate-800 text-indigo-650' : 'text-slate-500 bg-transparent'
                      }`}
                    >
                      Фикс наценка (руб)
                    </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-200">
              <div className="space-y-3.5 bg-white dark:bg-slate-950/40 p-4 border border-slate-205 dark:border-slate-800 rounded-xl shadow-2xs text-left">
                <h4 className="text-[11px] font-extrabold uppercase text-[#4C75A3] block">Исключение дубликатов публикаций</h4>
                <div className="space-y-2 text-left">
                  <label className="flex items-center gap-2 text-xs font-bold text-[#1f2937] dark:text-slate-300 cursor-pointer">
                    <input
                      type="checkbox"
              ...
              <div className="space-y-3.5 bg-white dark:bg-slate-950/40 p-4 border border-slate-205 dark:border-slate-800 rounded-xl shadow-2xs text-left">
                <h4 className="text-[11px] font-extrabold uppercase text-[#4C75A3] block">Задержка публикации (Анти-Капча)</h4>
                <div className="space-y-2 text-left">
                  <span className="text-[10px] font-extrabold text-slate-500 block uppercase">Пауза между публикациями:</span>
                  <select
                    ...
                    className="w-full text-xs p-2 border rounded bg-slate-50 dark:bg-slate-900 dark:border-slate-840 font-bold text-slate-800"
                  >
                    <option value={0}>Без задержки (Впритык к лимитам VK)</option>
              ...
              <div className="space-y-3.5 bg-white dark:bg-slate-950/40 p-4 border border-slate-205 dark:border-slate-800 rounded-xl shadow-2xs text-left">
                <h4 className="text-[11px] font-extrabold uppercase text-[#4C75A3] block">Дневной лимит выгрузок</h4>
                <div className="space-y-2 text-left">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-700 dark:text-slate-350">
                    ...
                    <span className="text-rose-500 dark:text-rose-400 font-black">{maxPostLimitPerDay} постов/день</span>
                  </div>
                  <input
                    type="range"
                    min="10"
                    ...
                    className="w-full h-1 bg-slate-200 rounded appearance-none cursor-pointer accent-indigo-600 mt-1 border-0 animate-pulse"
                  />
                  <p className="text-[10px] text-rose-500/80 leading-normal mt-1">
