 ...
 * Поддерживает разные форматы (response.items, raw array, response с профилями и группами).
 */
export function parseVkJson(
  ...
  try {
    ...
    // Извлекаем элементы постов, профили и группы
    let rawItems: any[] = [];
    let rawProfiles: any[] = [];
    let rawGroups: any[] = [];
    ...
    // Ограничиваем количество постов до 10 000 в соответствии с ТЗ (раздел Валидация и лимиты)
    const MAX_POSTS_LIMIT = 10000;
    const itemsToProcess = rawItems.slice(0, MAX_POSTS_LIMIT);

    // Создаем справочники для профилей пользователей и групп
    ...
    rawGroups.forEach((g) => {
      groupsMap.set(Number(g.id), g);
    });

    // Обрабатываем каждый пост и приводим его к общему типу VKPost
    const processedPosts: VKPost[] = [];

    for (const item of itemsToProcess) {
      // Игнорируем записи, не имеющие id (редкий случай для битых строк)
      if (!item.id) continue;
      ...
      const textEdited = textOriginal; // По умолчанию совпадает с исходным
      ...
      // Если цена найдена, рассчитываем нацененный вариант
